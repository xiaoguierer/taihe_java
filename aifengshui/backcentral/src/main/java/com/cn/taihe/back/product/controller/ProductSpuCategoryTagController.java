 ...
 */
@RestController
@RequestMapping("/product-spu-category")
...
public class ProductSpuCategoryTagController {
  ...
  public ResponseEntity<Object> createRealiations(
    @ApiParam(value = "SPU-商品标签关系信息", required = true)
    @RequestPart(value = "spuId",required = true)  String spuId,
    @RequestPart(value = "intentIds", required = true) List intentIds) {
    logger.info("开始处理新增SPU-商品标签关系请求, 操作人: {}, 参数: {}", OPERATOR, spuId);
    int result = productSpuCategoryTagService.createRealiations(spuId,intentIds);
