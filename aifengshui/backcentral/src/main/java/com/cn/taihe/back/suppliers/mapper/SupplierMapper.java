...
public interface SupplierMapper extends BaseMapper<Supplier> {
   ...
   */
  @Select("\n" +
    "SELECT s.* FROM suppliers s INNER JOIN product_spu_supplier pss ON s.id = pss.supplier_id " +
    "INNER JOIN product_spu_sku_rel psk ON pss.spu_id = psk.spu_id WHERE psk.sku_id = #{skuId} AND s.status = 1 ")
  List<Supplier> selectBySkuId(@Param("skuId") String skuId);
